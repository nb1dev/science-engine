...
class FormulationCalculator:
    ...
    def _distribute_into_capsules(self, components: List[Dict], count: int) -> List[Dict]:
        ...
        fill_mg includes the actual physical weight of mcg-dosed vitamins (converted to mg)
        for accurate display in decision traces and dashboards. These sub-mg weights are
        NOT counted during capacity optimization (correctly negligible for 650mg cap).
        """
        ...
        # Track actual physical weight separately (includes mcg→mg conversion)
        bin_display_totals = [0.0] * count

        for comp in sorted_comps:
            dose = comp.get("dose_mg", 0)
            if is_negligible_weight(comp.get("dose_unit", "mg")):
                # Negligible for capacity — put in first capsule
                bins[0].append(comp)
                # But add actual physical weight for display accuracy
                # mcg → mg conversion: dose_value (mcg) / 1000 = mg
                mcg_as_mg = comp.get("dose_value", 0) / 1000.0
                bin_display_totals[0] += mcg_as_mg
                continue
            ...
            bin_display_totals[best_bin] += dose

        result = []
        for i, (bin_comps, bin_total) in enumerate(zip(bins, bin_totals)):
            display_total = bin_display_totals[i]
            result.append({
                "capsule_number": i + 1,
                "components": bin_comps,
                "fill_mg": round(display_total, 1),
