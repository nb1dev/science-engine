...
def build_manufacturing_recipe(master: Dict) -> Dict:
    ...
    if mwc and mwc.get("components"):
        ...
        mwc_max_fill = round(sum(_mwc_fills) / max(len(_mwc_fills), 1), 1) if _mwc_fills else mwc_totals.get("total_weight_mg", 0)
        mwc_unit = {
            "unit_number": unit_num,
            "label": _l4,
            ...
            "fill_weight_per_capsule_mg": mwc_max_fill,  # per-capsule (for capacity validation)
    ...
    if ewc and ewc.get("components"):
        ...
        ewc_max_fill = round(sum(_ewc_fills) / max(len(_ewc_fills), 1), 1) if _ewc_fills else ewc_totals.get("total_weight_mg", 0)
        ewc_unit = {
            "unit_number": unit_num,
            "label": "Evening Wellness Capsule",
            "format": {"type": "hard_capsule", "size": "00", "material": "vegetarian"},
            ...
            "fill_weight_per_capsule_mg": ewc_max_fill,  # per-capsule (for capacity validation)
