...
def _highlight_claims_in_text(text, claims_set):
    ...
    for claim in claims_set:
        # Try both the claim itself and individual words
        keywords = [claim.lower()]
        if len(claim.split()) > 1:
            keywords.extend(w.lower() for w in claim.split() if len(w) > 3)
        for kw in keywords:
            pattern = re.compile(r'(?i)\b(' + re.escape(kw) + r')\b')
            result = pattern.sub(r'<mark style="background:var(--purple-lt);color:var(--purple);padding:0 3px;border-radius:3px;font-weight:600">\1</mark>', result)
    return result
...
def build_board_dashboard(sample_id: str, output_dir: str) -> str:
  ...
  <div class="cover-body">
    ...
    stats_html = "".join(f'<div style="padding:8px 0;border-bottom:1px solid var(--rule);display:flex;justify-content:space-between"><span style="font-size:11px;letter-spacing:.1em;text-transform:uppercase;color:var(--soft)">{k}</span><span style="font-size:13px;color:var(--dark);font-weight:500">{v}</span></div>' for k, v in stat_items)
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:32px;align-items:start">
    <div>
      <div style="font-size:10px;letter-spacing:.25em;text-transform:uppercase;color:var(--soft);margin-bottom:12px">Microbiome Analysis</div>
      <p style="font-size:13px;color:var(--mid);line-height:1.8;margin-bottom:20px">{_esc(inputs["microbiome"]["summary"])}</p>
      <div class="guild-bars">{guild_bars_html}</div>
      <div style="margin-top:14px">{clr_html}</div>
    </div>
    <div>
      <div style="font-size:10px;letter-spacing:.25em;text-transform:uppercase;color:var(--soft);margin-bottom:12px">Questionnaire Profile</div>
      <p style="font-size:13px;color:var(--mid);line-height:1.8;margin-bottom:20px">{_esc(q_narrative or inputs.get("questionnaire",{}).get("summary",""))}</p>
      <div>{stats_html}</div>
    </div>
  </div>
  {formulation_logic_html}
    ...
    for step in steps:
        ...
        if step.get("decision") == "Prebiotic Design":
            ...
            if pb_data:
                ...
                pb_items = "".join(f'<div style="font-size:12px;color:var(--mid);padding:2px 0">→ {_esc(p["substance"])}: {p["dose_g"]}g {_fodmap_span if p.get("fodmap") else ""}</div>' for p in pb_data)
                extra_detail = f'<div style="margin-top:8px;background:var(--sand);border-radius:6px;padding:10px 12px">{pb_items}</div>'
            # Strategy & overrides
            strategy = master.get("decisions", {}).get("prebiotic_design", {}).get("strategy", "")
            overrides = master.get("decisions", {}).get("prebiotic_design", {}).get("overrides_applied", [])
            if strategy:
        ...
        # Input goals highlighting
        input_text = _esc(step.get("input", ""))
        # Highlight goal keywords in input
        for g in q_goals:
            for word in g.replace("_", " ").split():
                if len(word) > 3:
                    pattern = re.compile(r'(?i)\b(' + re.escape(word) + r')\b')
                    input_text = pattern.sub(r'<span style="background:var(--green-lt);color:var(--green);padding:0 3px;border-radius:3px;font-weight:600">\1</span>', input_text)
