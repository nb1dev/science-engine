...
DELIVERY STRUCTURE (for dose optimization):
- Morning sachet (19g max): prebiotics (~5-8g), water-soluble vitamins, minerals (except Mg), amino acids, sachet-safe botanicals
- Evening capsule (650mg max): calming adaptogens (Ashwagandha, etc.), sleep aids, Tier 1 polyphenols (Quercetin, Propolis)
- Polyphenol capsule (650mg max, morning): Curcumin+Piperine, Bergamot — large polyphenols get their own capsule
- Capsule-only substances (Ashwagandha, Rhodiola, Curcumin, Quercetin, Propolis, Bacopa, Ginger Root, Star Anise, Bergamot, Capsicum) go in capsules, NOT sachet
When selecting capsule-only substances, you have UP TO 650mg in the evening capsule — don't limit their doses based on sachet constraints. Use the UPPER end of the KB therapeutic range when clinically appropriate. For example, Ashwagandha 600mg is preferred over 300mg when stress ≥6/10 and capsule space allows.
...
- Consider the total sachet weight (19g max) — prebiotics need 4-7g, vitamins ~0.3g, so supplements should fit in the remaining space
- DO NOT add a supplement if its primary benefit is already fully covered by another selected supplement with the same mechanism, UNLESS it has a genuinely complementary mechanism (e.g., Fennel for gas reduction + Peppermint for cramping = different mechanisms, OK to combine)
...
- Check for supplement-to-supplement absorption conflicts (e.g., Zinc and Calcium compete for absorption — avoid co-selecting if possible, or note the spacing requirement)
  ...
  "supplements": [
    {
      ...
      "delivery": "<sachet|evening_capsule>",
...
def select_supplements(unified_input: Dict, rule_outputs: Dict) -> Dict:
    ...
    evening_headroom_note = f"Evening capsule: {available_evening_mg}mg available"
    if reserved_evening_items:
        evening_headroom_note += f" (650mg capacity minus {reserved_evening_mg}mg reserved for deterministic sleep supplements: {', '.join(reserved_evening_items)})"
    else:
        evening_headroom_note += " (650mg capacity, no sleep supplements reserved)"
...
Your task: Design the prebiotic blend for the sachet.
  ...
  "prebiotics": [
    {
      ...
      "dose_g": <number>,
      "fodmap": <true|false>,
      "rationale": "<why this prebiotic and dose>"
    }
  ],
...
def design_prebiotics(unified_input: Dict, rule_outputs: Dict, mix_selection: Dict) -> Dict:
    """LLM Call 3: Design prebiotic blend for sachet."""
...
Design the prebiotic blend. Check for contradictions and apply overrides if needed.
...
def run_llm_decisions(
    ...
    if mix_result.get("mix_id"):
        ...
        if kb_strains:
            ...
            print(f"    Strains: {len(kb_strains)} from KB ({cfu_per_strain}B each) + {len(existing_lp815)} enhancements")
...
def _build_sanity_check_kb_references() -> str:
    ...
    try:
        ...
        for fmt_key, fmt_data in delivery_kb.get("delivery_formats", {}).items():
            ...
            capacity_g = fmt_data.get("capacity_g")
            ...
            daily_total_str = ""
            if capacity_mg and daily_count > 1:
                daily_total_str = f" → daily total: {capacity_mg * daily_count}mg across {daily_count} units"
            elif capacity_g and daily_count > 1:
                daily_total_str = f" → daily total: {capacity_g * daily_count}g across {daily_count} units"

            fixed_str = " [FIXED — identical for all clients]" if fixed else ""
            delivery_lines.append(
                f"- {label}: capacity={cap_str}/unit, daily_count={daily_count}, timing={timing}{daily_total_str}{fixed_str}"
            )
...
def formulation_sanity_check(
    ...
    for unit in recipe.get("units", []):
        ...
        if label.lower() in FIXED_UNIT_LABELS:
            skipped_fixed.append(label)
            continue
        timing = unit.get("timing", "?")
        fill_mg = unit.get("fill_weight_mg") or unit.get("total_weight_mg")
